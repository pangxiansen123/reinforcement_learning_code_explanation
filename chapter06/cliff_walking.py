...
def step(state, action):
    ...
    reward = -1
    # 就是第三行的第二列到第十列，然后在起点的时候，向右移动也是掉进悬崖
...
def sarsa(q_value, expected=False, step_size=ALPHA):
    ...
    rewards = 0.0
    # 先目标策略，更新Q，再行动策略，只是这里的目标策略是用到平均值
    while state != GOAL:
        # 这个reward就是R(t+1)
        ...
        rewards += reward
        if not expected:
            # 这个就是Q(S',A') 就是普通的sarsa
            target = q_value[next_state[0], next_state[1], next_action]
        else:
            # calculate the expected value of new state 这个是期望sarsa
            target = 0.0
            q_next = q_value[next_state[0], next_state[1], :]
            best_actions = np.argwhere(q_next == np.max(q_next))
            for action_ in ACTIONS:
                if action_ in best_actions:
                    # ((1.0 - EPSILON) / len(best_actions) + EPSILON / len(ACTIONS))就是选择最优的概率
...
# @q_value: values for state action pair, will be updated
# @step_size: step size for updating
# @return: total rewards within this episode 这一幕数据的G
