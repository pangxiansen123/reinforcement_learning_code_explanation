...
class PolicyIteration:
    ...
    def solve(self):
        ...
        while True:
            ...
            print('PE => Elapsed time {} seconds'.format(elapsed_time))
            start_time = time.time()

            policy_change, self.policy = self.policy_improvement(self.actions, self.values, self.policy)
            elapsed_time = time.time() - start_time
            print('PI => Elapsed time {} seconds'.format(elapsed_time))
            if policy_change == 0:
                break
            iterations += 1
        total_elapsed_time = time.time() - total_start_time
        print('Optimal policy is reached after {0} iterations in {1} seconds'.format(iterations,total_elapsed_time))
    ...
    def policy_improvement(self, actions, values, policy):
        ...
        print('Policy changed in {} states'.format(policy_change))
