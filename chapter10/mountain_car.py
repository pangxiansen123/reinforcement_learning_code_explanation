...
class IHT:
    ...
    # 得到obj所对应的值，如果obj键不存在，并且read_only为true，那么就返回空，如果read_only为false，
    # 那么就创建一个键值对，值是字典的长度
    # 返回键所对应的值
    def get_index(self, obj, read_only=False):
        ...
        if count >= size:
            ...
            d[obj] = count
            return count
# coordinates[哪个覆盖 位置 速度 动作] m就是IHT类的对象
def hash_coords(coordinates, m, read_only=False):
    # isinstance() 函数来判断一个对象是否是一个已知的类型，类似 type()。isinstance() 会认为子类是一种父类类型，考虑继承关系。
    # 就是判断m是否是IHT类型
    # 这个应该就是执行这个，后面的if应该不执行
    # 把列表转化成元组，为什么用元组，就是因为字典里面的键是不可以更改的，所以要使用元组
...
def step(position, velocity, action):
    ...
    reward = -1.0
    # 书上进行说明，当到达最右端的时候，我们会将速度置零，但是实在这个位置不变化
...
class ValueFunction:
    ...
    # (positions[update_time], velocities[update_time], actions[update_time], returns)
    def learn(self, position, velocity, action, target):
        # 得到使position, velocity, action活跃的瓦片的下标
        # 应该是八个元素
        active_tiles = self.get_active_tiles(position, velocity, action)
...
def semi_gradient_n_step_sarsa(value_function, n=1):
    ...
    # get initial action
    # 这个是通过贪心算法得到最优的动作，所以这里面没有用到eplison-贪心
